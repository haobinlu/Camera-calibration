...
from src_test.utils import Utils
from src_test.my_thread.RGB_1_Thread import RGB_1_Thread
from src_test.my_thread.RGB_2_Thread import RGB_2_Thread
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def check_compelet(self):
        if self.RGB_1_th.compelet and self.RGB_2_th.compelet:
            ...
            self.ui.label_status.setText('相片采集完成,开始对齐')
            mean_delay = self.utils.alignment()
            if mean_delay == None:
                self.ui.label_status.setStyleSheet("color:{}".format('red'))
                self.ui.label_status.setText('可用帧数达不到要求')
            else:
                self.ui.label_status.setText('采集完成,平均延迟为{}ms'.format(round(mean_delay, 2) ))
    ...
    def save_frame_changed(self):
        try:
            self.save_frame = int(self.ui.lineEdit_save_frame.text())
            print('将保存帧数改成了{}'.format(self.save_frame))
        except:
            return
    ...
    def __init__(self):
        ...
        其他设置
        '''
        self.sample_frame = int(self.ui.lineEdit_sample_frame.text())  #采样帧数
        self.save_frame = int(self.ui.lineEdit_save_frame.text())  # 最后保存的帧数
        ...
        self.conf.save_frame = self.save_frame
        self.conf.sample_frame = self.sample_frame
        self.conf.current_NO = self.current_NO
        self.conf.current_ID = self.current_ID
        self.conf.current_sex = self.current_sex
        ...
        self.ui.lineEdit_save_frame.textChanged.connect(self.save_frame_changed)
