...
mainthread_process::mainthread_process(int cam) {
	this->cam = cam;

}


void mainthread_process::run() {
	while (1) {
		if (!isRunning) {
			QThread::msleep(10);
			continue;
		...
		else
		{
			...
			if (issave) {
				...
				r_t += 1;
				emit recordProgress((int)(r_t * 100 / record_frame));
				if (r_t == record_frame) {
					utils->save_videos(frame_ls, FrameNum_ls, cam);
					r_t = 0;
					issave = false;
					compelet = true;
					FrameNum_cam2 = FrameNum_ls;
					frame_ls.clear();
					FrameNum_ls.clear();
					utils->add_id();

					QThread::sleep(1);
					emit signal_check_compelet(true);
				}
			}
		}
	}
